...
public class DefaultClientConnectionManagerMonitor implements IdleConnectionMonitor.ClientConnectionManagerMonitor {
    ...
    public void monitor(ClientConnectionManager cm) {
		CleanupTask cleanupTask = new CleanupTask(cm);
        ScheduledFuture<?> cleanupFuture = executorService.scheduleWithFixedDelay(cleanupTask, DEFAULT_IDLE_CHECK_INTERVAL, DEFAULT_IDLE_CHECK_INTERVAL, TimeUnit.SECONDS);
